/**
 * @file benchmarks/codelet_polybench_fdtd_2d_c_init_array_line38_0.cpp
 * @brief Benchmark extracted from the Polybench codelet
 */

#include "bench_common.hpp"

int i = 500;
int nx = 1000;
int j = 0;
int ny = 1200;
double ex[1000 + 0][1200 + 0];
double ey[1000 + 0][1200 + 0];
double hz[1000 + 0][1200 + 0];

BENCHMARK_FN(fdtd_2d_c_init_array_line38_loop_c_0_c, {
	int __i_0__ = i;
	int __j_1__ = j;

	{
		for (__i_0__ = 0; __i_0__ <= nx - 1; __i_0__ += 1) {
			for (__j_1__ = 0; __j_1__ <= ny - 1; __j_1__ += 1) {
				ex[__i_0__][__j_1__] = ((double)__i_0__) * (__j_1__ + 1) / nx;
				ey[__i_0__][__j_1__] = ((double)__i_0__) * (__j_1__ + 2) / ny;
				hz[__i_0__][__j_1__] = ((double)__i_0__) * (__j_1__ + 3) / nx;
			}
		}
	}

	i = __i_0__;
	j = __j_1__;
})

/**
 * @brief Run the benchmarks for fdtd_2d_c_init_array_line38_loop_c_0_c
 */
int main() {
	std::ostringstream oss;
	auto result = BENCHMARK_INIT(10, oss).RUN(fdtd_2d_c_init_array_line38_loop_c_0_c).results();

	// std::cout << oss.str();

	for (auto const& res : result) {
		auto measure = res.fromString("cpucycles");
		auto name = res.config().mBenchmarkName;
		std::cout << name << " : ";
		std::cout << res.median(measure) << " cycles\n";
	}

	return 0;
}