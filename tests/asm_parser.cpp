...
TEST(asm_parser, Single_Instruction) {
	...
	result = AsmParser().parse_in(instruction);
	EXPECT_EQ(result, expected);
}
 ...
 */
TEST(asm_parser, Multiple_Instructions) {
 ...
 */
TEST(asm_parser_out, Multiple_Instructions) {
	std::string instructions = ";; Cycles : 1002\n"
							   "imul eax, ebx;\n"
							   "imul ecx, edx;\n"
							   "add eax, ecx;\n"
